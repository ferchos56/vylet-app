...
export default function SelectorCiudades() {
  ...
  const cargarCiudades = async (paginaSolicitada) => {
    ...
    setCargando(true);
    try {
      const res = await fetch(`${API_URL}/ciudades/delante/?pagina=${paginaSolicitada}&limite=8`);
        ...
        >
          {filtrarCiudades().map(ciudad => (
            <TouchableOpacity
              ...
              style={styles.card}
              onPress={() => navigation.navigate('Ciudades', { ciudad })}
            >
              <Image
                source={{ uri: `${API_URL}/ciudades/delante/${ciudad.imagen}` }}
          ...
          {cargando && <ActivityIndicator size="large" color="#fff" style={{ marginTop: 10 }} />}
        </ScrollView>
      </View>
    </SafeAreaView>
  );
